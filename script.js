...
setInterval(() => {
    ...
    if(x<=510){
        ...
        alert("LOSER !! YOUR SCORE IS: "+score);
...
function checkcollision(elm1,elm2){
    var elm1Rect=elm1.getBoundingClientReac();
    var elm2Rect=elm2.getBoundingClientReac();

    return(elm1Rect.right>=elm2Rect.left &&
         (elm1Rect.left<=elm1Rect.right) && 
         (elm1Rect.bottom>=elm2Rect.bottom>=elm2Rect.top) &&
         (elm1Rect.left<=elm2Rect.bottom));
}
